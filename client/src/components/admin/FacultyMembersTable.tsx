...
export function FacultyMembersTable({ faculty, onEdit }: FacultyMembersTableProps) {
  const { toast } = useToast();
  const queryClient = useQueryClient();
  const [expandedId, setExpandedId] = useState<number | null>(null);

  const deleteMutation = useMutation({
    mutationFn: (id: number) => apiRequest("DELETE", `/api/university/faculty/${id}`, {}),
    onSuccess: () => {
                    ...
                    <div className="mt-4 bg-muted/50 p-4 rounded-md">
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                        ...
                        <div className="md:col-span-2">
                          ...
                          <p className="text-sm font-medium mt-2">Specialization:</p>
